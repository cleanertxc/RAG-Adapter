...
The notebooks retain the original research logic and include independent experiment sections. Read [the notebook guide](docs/notebooks.md) before executing cells. This source release does not include the complete environment, fine-tuned checkpoints or per-run configurations for every reported experiment. No paper results have been recomputed during repository preparation.
...
The [sampling record guide](data/sampled_records/README.md) describes the four CSV files and video lists. Videos and model weights are obtained separately from their providers.
